...
bool is_memory_trap(uintptr_t addr) {
...
int main()
{
	auto pid = smemory::initalize_by_game_package_return_pid("com.tencent.tmgp.sgame"); // initalize by game package
	
	auto base =smemory::get_module_base_pro("libGameCore.so",1,"rw");
	printf("进程pid %d %lx\n",pid,base);
	init_pid(pid); // 进程读写需要的初始化pid
    
	//rw_function.insert({"kernel_read",vm_readv});
	//rw_function.insert({"kernel_write",vm_writev});
	smemory::memory_debug(true); //打印自定义读写实例
	
    bool m = is_memory_trap(base);
    printf("缺页 %d\n",m);
	auto i = smemory::read_int(base);
	printf("读测试 %d \n",i);
	
	smemory::write_int(base,1);
	auto i2 = smemory::read_int(base);
	printf("读测试 %d \n",i2);
	return 0;
}